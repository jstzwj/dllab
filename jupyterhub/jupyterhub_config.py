...
#  Default: 'http://:8000'
c.JupyterHub.bind_url = 'http://' + os.environ['HOST'] + ':8000'
...
#  Default: 'C:\\Users\\wangjun\\.conda\\envs\\directml\\share\\jupyterhub'
# c.JupyterHub.data_files_path = 'C:\\Users\\wangjun\\.conda\\envs\\directml\\share\\jupyterhub'
...
# c.DockerSpawner.extra_create_kwargs = { 'network_mode':  network_name }
...
internal_gid = os.environ.get('INTERNAL_GID')
internal_uid = os.environ.get('INTERNAL_UID')
internal_uname = os.environ.get('INTERNAL_USERNAME')

c.DockerSpawner.extra_create_kwargs = {
    'user': 'root',
    'working_dir': '/home/' + internal_uname,
}
...
def create_dir_hook(spawner):
    ...
    quoted_username = quoted_username.replace("_", "-5f")
    quoted_username = quoted_username.replace("%", "-")
    # path on the jupytherhub host, create a folder based on username if not exists
    volume_path = os.path.join("/srv/notebook_data", f'jupyterhub-user-{quoted_username}')
    work_path = os.path.join(volume_path, 'work')
    if not os.path.exists(volume_path):
        os.mkdir(volume_path)
        os.mkdir(work_path)
        # shutil.chown(volume_path, user=uname, group=uname)
        subprocess.check_call(["cp", "/etc/skel/.bashrc", volume_path])
        subprocess.check_call(["cp", "/etc/skel/.profile", volume_path])
        # subprocess.check_call(["chown", "-R", f"{internal_uid}:{internal_gid}", volume_path])

c.Spawner.pre_spawn_hook = create_dir_hook
c.DockerSpawner.remove_containers = True

c.DockerSpawner.notebook_dir = notebook_dir
home_dir = "/home/" + internal_uname
...
#  Default: ''
c.Authenticator.username_pattern = '[a-zA-Z][a-zA-Z0-9_]{4,}'
